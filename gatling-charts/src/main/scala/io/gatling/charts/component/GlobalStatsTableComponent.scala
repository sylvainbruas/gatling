...
"""
  ...
  private def generateHtmlRow(container: Container, level: Int, index: Int, parent: Option[String], group: Boolean): String = {
    val fileName = if (group) container.stats.path.toGroupFileName else container.stats.path.toRequestFileName
    val (url, name) = container match {
      case groupContainer: GroupContainer if groupContainer.group == Group.Root => ("index.html", "All Requests")
      case _                                                                    => (s"$fileName.html", container.name)
    }

    val expandButtonStyle = if (group) "" else " hidden"
    ...
    s"""<tr $dataParent>
       ...
       |      <span id="$fileName" style="margin-left: ${level * 10}px;" class="expand-button$expandButtonStyle">&nbsp;</span>
       |        <a href="$url" class="withTooltip">
       |          <span class='table-cell-tooltip' id='parent-$name' data-toggle='popover' data-placement='right' data-container='body' data-content=''>
       |            <span onmouseover='isEllipsed("$name")' id='$name' class='ellipsed-name'>$name</span>
       |          </span>
       |        </a>
